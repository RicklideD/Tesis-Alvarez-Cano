...
class SpiderLanguage:
    ...
    def __init__(
        ...
        # recopilar punteros y comprobadores
        ...
        if self.clause_order:
            # el orden de las cláusulas se prioriza sobre configuraciones como end_with_from
            assert factorize_sketch == 2 #TODO apoya otras gramáticas
    ...
    def tokenize_field_value(cls, field_value):
        ...
        # TODO: Eliminar comillas circundantes
...
class SpiderUnparser:
    ...
    def refine_from(self, tree):
        """
        1) Inferir tablas a partir de columnas predichas 
	2) Mezclarlas con las tablas predichas si las hay 
	3) Inferir condiciones basadas en las tablas  
        """
        tree = dict(tree)

        # consulta anidada en la cláusula from, usar recursivamente el refinamiento
        ...
        if "from" in tree:
            ...
            tree["from"]["table_units"] = table_unit_set # remover duplicados

        # Obtener todas las columnas candidatas
        ...
        # Tomar la unión de tablas inferidas y tablas predichas
        all_from_table_ids = must_in_from_table_ids.union(predicted_from_table_ids)
        if not all_from_table_ids:
            # TODO:  Mejor heurística, por ejemplo, tablas que tengan coincidencia exacta
    ...
    def unparse_from(self, from_):
        ...
        for i, table_unit in enumerate(from_.get('table_units', [])):
            ...
            if table_unit['_type'] == 'TableUnitSql':
                ...
                # Output "ON <cond>" si todas las tablas involucradas en la condición han sido emitidas
                conds_to_output = []
