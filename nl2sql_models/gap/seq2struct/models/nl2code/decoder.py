...
def get_field_presence_info(ast_wrapper, node, field_infos):
    ...
    for field_info in field_infos:
        ...
        if maybe_missing and is_builtin_type:
            # TODO: # TODO: hacer posible manejar "singleton?"
...
class NL2CodeDecoderPreproc(abstract_preproc.AbstractPreproc):
    ...
    def _record_productions(self, tree):
        ...
        while queue:
            ...
            # Reglas de la forma:
            # FunctionDef
            # -> identificador name, argumentos args
            # |  identificador name, argumentos args, stmt* body
            # |  identificador name, argumentos args, expr* decorator_list
            # |  identificador name, argumentos args, expr? returns
            # ...
            # |  identificador name, argumentos args, stmt* body, expr* decorator_list, expr returns
...
class NL2CodeDecoder(torch.nn.Module):
    ...
    def __init__(
            ...
            rule_emb_size=128,
            node_embed_size=64,
            # TODO: Esto debería inferirse automáticamente desde el codificador
        ...
        if desc_attn == 'bahdanau':
            ...
            # TODO: Descubrir cómo obtener tamaños correctos (consulta, valor) al módulo
        ...
        if copy_pointer is None:
            ...
            # Descubrir cómo obtener tamaños correctos (consulta, valor) al módulo
    ...
    def compute_mle_loss(self, enc_input, example, desc_enc, debug=False):
        ...
        while queue:
            ...
            if parent_field_type in self.ast_wrapper.primitive_types:
                # identificador, int, string, bytes, object, singleton
                # - podría ser bytes, str, int, float, bool, NoneType
                # - l vocabulario de tokens terminales se crea convirtiendo todo en una cadena (con `str`)
                # - en el momento de decodificar, se convierte de nuevo a str/int/float/bool
            ...
            if type_info.fields:
                # ApplyRule, como  Call -> expr[func] expr*[args] keyword*[keywords]
                # Descubrir qué regla debe aplicarse
            ...
            # invertido para que realicemos un DFS en orden de izquierda a derecha
            for field_info in reversed(type_info.fields):
    ...
    def rule_infer(self, node_type, rule_logits):
        ...
        # TODO: ¿Máscara de otras probabilidades primero?
        return list(zip(
    ...
    def gen_token(
        ...
        new_state, attention_logits = self._update_state(
            node_type, prev_state, prev_action_emb, parent_h, parent_action_emb, desc_enc)
        # forma de output: batch (=1) x emb_size
        output = new_state[0]

        # forma de gen_logodds: batch (=1)
    ...
    def gen_token_loss(
        ...
        if desc_locs:
            ...
            copy_logprob = (
                ...
                # TODO: sumar la probabilidad de todas las ocurrencias
                # shape: batch (=1)
        ...
        # la pérdida debe ser -log p(...), así que negar
    ...
    def token_infer(self, output, gen_logodds, desc_enc):
        ...
        log_prob_by_word = {}
        # accumulate_logprobs es necesario porque la misma palabra puede aparecer
        # varias veces en desc_enc.words.
        accumulate_logprobs(
            log_prob_by_word,
    ...
    def compute_pointer(
        ...
        new_state, attention_logits = self._update_state(
            node_type, prev_state, prev_action_emb, parent_h, parent_action_emb, desc_enc)
        # forma de salida: batch (=1) x emb_size
        output = new_state[0]
