...
class TreeTraversal:
    ...
    def process_sum_inquire(self, last_choice):
        # 1. ApplyRule, como expr -> Call
        # a. Preguntar cuál elegir
    ...
    def process_sum_apply(self, last_choice):
        # b. Agregar acción, prepararse para #2
    ...
    def process_children_inquire(self, last_choice):
        # 2. ApplyRule, como Call -> expr[func] expr*[args] keyword*[keywords]
        # Verificar si no hay hijos
        ...
        # a. Preguntar sobre la presencia
        output, self.recurrent_state, rule_logits = self.model.apply_rule(
            self.cur_item.node_type,
            self.recurrent_state,
            self.prev_action_emb,
    ...
    def process_children_apply(self, last_choice):
        ...
        for field_info, present in reversed(
            ...
            # campo seq: LIST_LENGTH_INQUIRE x
            # tipo suma: SUM_TYPE_INQUIRE x
            # tipo producto:
            #   sin hijos: not possible
            #   con hijos: CHILDREN_INQUIRE
            # tipo constructor: not possible x
            # tipo incorporado: GEN_TOKEN x
